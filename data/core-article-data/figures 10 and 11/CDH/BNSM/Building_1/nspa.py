...
def do_nspa_x(max_drift: float = 0.1, dincr: float = 0.001) -> tuple[list[float], list[float]]:
    ...
    ops.load(91000, 0.1108749238980588, 0, 0, 0, 0, 0)
    ops.load(92000, 0.2386696608728905, 0, 0, 0, 0, 0)
    ops.load(93000, 0.3410923879775832, 0, 0, 0, 0, 0)
    ops.load(94000, 0.3093630272514676, 0, 0, 0, 0, 0)
...
def do_nspa_y(max_drift: float = 0.1, dincr: float = 0.001) -> tuple[list[float], list[float]]:
    ...
    ops.pattern('Plain', 2, 2)
    # Add lateral nspa loads to ops domain
    ops.load(91000, 0, 0.11008782067525333, 0, 0, 0, 0)
    ops.load(92000, 0, 0.23838945389550184, 0, 0, 0, 0)
    ops.load(93000, 0, 0.34162699236766586, 0, 0, 0, 0)
    ops.load(94000, 0, 0.30989573306157897, 0, 0, 0, 0)
