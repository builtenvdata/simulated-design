...
def do_nspa_x(max_drift: float = 0.1, dincr: float = 0.001) -> tuple[list[float], list[float]]:
    ...
    ops.load(91000, 0.1169411777589394, 0, 0, 0, 0, 0)
    ops.load(92000, 0.23394694783813916, 0, 0, 0, 0, 0)
    ops.load(93000, 0.33725159632623924, 0, 0, 0, 0, 0)
    ops.load(94000, 0.31186027807668215, 0, 0, 0, 0, 0)
...
def do_nspa_y(max_drift: float = 0.1, dincr: float = 0.001) -> tuple[list[float], list[float]]:
    ...
    ops.pattern('Plain', 2, 2)
    # Add lateral nspa loads to ops domain
    ops.load(91000, 0, 0.10943406972978607, 0, 0, 0, 0)
    ops.load(92000, 0, 0.23394028912203607, 0, 0, 0, 0)
    ops.load(93000, 0, 0.34284261050305836, 0, 0, 0, 0)
    ops.load(94000, 0, 0.31378303064511953, 0, 0, 0, 0)
