...
def do_nspa_x(max_drift: float = 0.1, dincr: float = 0.0001) -> tuple[list[float], list[float]]:
    ...
    ops.load(91000, 0.0998466090195044, 0, 0, 0, 0, 0)
    ops.load(92000, 0.20069653943848545, 0, 0, 0, 0, 0)
    ops.load(93000, 0.3449070257761597, 0, 0, 0, 0, 0)
    ops.load(94000, 0.3545498257658505, 0, 0, 0, 0, 0)
...
def do_nspa_y(max_drift: float = 0.1, dincr: float = 0.0001) -> tuple[list[float], list[float]]:
    ...
    ops.pattern('Plain', 2, 2)
    # Add lateral nspa loads to ops domain
    ops.load(91000, 0, 0.10513914993605962, 0, 0, 0, 0)
    ops.load(92000, 0, 0.2095158040549616, 0, 0, 0, 0)
    ops.load(93000, 0, 0.34613932397559816, 0, 0, 0, 0)
    ops.load(94000, 0, 0.3392057220333806, 0, 0, 0, 0)
