...
def do_nspa_x(max_drift: float = 0.1, dincr: float = 0.0001) -> tuple[list[float], list[float]]:
    ...
    ops.load(91000, 0.11232595137194441, 0, 0, 0, 0, 0)
    ops.load(92000, 0.21353512623289622, 0, 0, 0, 0, 0)
    ops.load(93000, 0.33622363624200274, 0, 0, 0, 0, 0)
    ops.load(94000, 0.33791528615315664, 0, 0, 0, 0, 0)
...
def do_nspa_y(max_drift: float = 0.1, dincr: float = 0.0001) -> tuple[list[float], list[float]]:
    ...
    ops.pattern('Plain', 2, 2)
    # Add lateral nspa loads to ops domain
    ops.load(91000, 0, 0.11071787558021898, 0, 0, 0, 0)
    ops.load(92000, 0, 0.2162775694327755, 0, 0, 0, 0)
    ops.load(93000, 0, 0.34130456270107296, 0, 0, 0, 0)
    ops.load(94000, 0, 0.3316999922859325, 0, 0, 0, 0)
