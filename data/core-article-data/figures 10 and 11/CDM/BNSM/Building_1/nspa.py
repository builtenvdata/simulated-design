...
def do_nspa_x(max_drift: float = 0.1, dincr: float = 0.0001) -> tuple[list[float], list[float]]:
    ...
    ops.load(91000, 0.11912099612098574, 0, 0, 0, 0, 0)
    ops.load(92000, 0.23470869118042925, 0, 0, 0, 0, 0)
    ops.load(93000, 0.3406488737989223, 0, 0, 0, 0, 0)
    ops.load(94000, 0.30552143889966277, 0, 0, 0, 0, 0)
...
def do_nspa_y(max_drift: float = 0.1, dincr: float = 0.0001) -> tuple[list[float], list[float]]:
    ...
    ops.pattern('Plain', 2, 2)
    # Add lateral nspa loads to ops domain
    ops.load(91000, 0, 0.1213140645742421, 0, 0, 0, 0)
    ops.load(92000, 0, 0.2332627534498659, 0, 0, 0, 0)
    ops.load(93000, 0, 0.33829887321074303, 0, 0, 0, 0)
    ops.load(94000, 0, 0.30712430876514896, 0, 0, 0, 0)
