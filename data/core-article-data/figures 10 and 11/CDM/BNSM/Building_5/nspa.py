...
def do_nspa_x(max_drift: float = 0.1, dincr: float = 1e-05) -> tuple[list[float], list[float]]:
    ...
    ops.load(91000, 0.120036486180096, 0, 0, 0, 0, 0)
    ops.load(92000, 0.22374048450117823, 0, 0, 0, 0, 0)
    ops.load(93000, 0.33307878901097404, 0, 0, 0, 0, 0)
    ops.load(94000, 0.32314424030775174, 0, 0, 0, 0, 0)
...
def do_nspa_y(max_drift: float = 0.1, dincr: float = 1e-05) -> tuple[list[float], list[float]]:
    ...
    ops.pattern('Plain', 2, 2)
    # Add lateral nspa loads to ops domain
    ops.load(91000, 0, 0.12305229590379092, 0, 0, 0, 0)
    ops.load(92000, 0, 0.22448273895424856, 0, 0, 0, 0)
    ops.load(93000, 0, 0.3326177099497944, 0, 0, 0, 0)
    ops.load(94000, 0, 0.319847255192166, 0, 0, 0, 0)
