...
def do_nspa_x(max_drift: float = 0.1, dincr: float = 0.001) -> tuple[list[float], list[float]]:
    ...
    ops.load(91000, 0.11009977951809391, 0, 0, 0, 0, 0)
    ops.load(92000, 0.23071342415992546, 0, 0, 0, 0, 0)
    ops.load(93000, 0.34585795910236433, 0, 0, 0, 0, 0)
    ops.load(94000, 0.3133288372196163, 0, 0, 0, 0, 0)
...
def do_nspa_y(max_drift: float = 0.1, dincr: float = 0.001) -> tuple[list[float], list[float]]:
    ...
    ops.pattern('Plain', 2, 2)
    # Add lateral nspa loads to ops domain
    ops.load(91000, 0, 0.11890172634843407, 0, 0, 0, 0)
    ops.load(92000, 0, 0.23124446440548072, 0, 0, 0, 0)
    ops.load(93000, 0, 0.3429506169844193, 0, 0, 0, 0)
    ops.load(94000, 0, 0.30690319226166585, 0, 0, 0, 0)
