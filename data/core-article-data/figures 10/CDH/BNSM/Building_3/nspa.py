...
def do_nspa_x(max_drift: float = 0.1, dincr: float = 0.001) -> tuple[list[float], list[float]]:
    ...
    ops.load(91000, 0.11516888164929748, 0, 0, 0, 0, 0)
    ops.load(92000, 0.22930479920850136, 0, 0, 0, 0, 0)
    ops.load(93000, 0.3386778275118842, 0, 0, 0, 0, 0)
    ops.load(94000, 0.316848491630317, 0, 0, 0, 0, 0)
...
def do_nspa_y(max_drift: float = 0.1, dincr: float = 0.001) -> tuple[list[float], list[float]]:
    ...
    ops.pattern('Plain', 2, 2)
    # Add lateral nspa loads to ops domain
    ops.load(91000, 0, 0.10994096643844833, 0, 0, 0, 0)
    ops.load(92000, 0, 0.22925752160255777, 0, 0, 0, 0)
    ops.load(93000, 0, 0.3392860758249067, 0, 0, 0, 0)
    ops.load(94000, 0, 0.32151543613408723, 0, 0, 0, 0)
