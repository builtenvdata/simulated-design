...
def do_nspa_x(max_drift: float = 0.1, dincr: float = 0.001) -> tuple[list[float], list[float]]:
    ...
    ops.load(91000, 0.10990187454035497, 0, 0, 0, 0, 0)
    ops.load(92000, 0.23236665015867064, 0, 0, 0, 0, 0)
    ops.load(93000, 0.33578285354587806, 0, 0, 0, 0, 0)
    ops.load(94000, 0.32194862175509625, 0, 0, 0, 0, 0)
...
def do_nspa_y(max_drift: float = 0.1, dincr: float = 0.001) -> tuple[list[float], list[float]]:
    ...
    ops.pattern('Plain', 2, 2)
    # Add lateral nspa loads to ops domain
    ops.load(91000, 0, 0.10641148352879704, 0, 0, 0, 0)
    ops.load(92000, 0, 0.22762625634984976, 0, 0, 0, 0)
    ops.load(93000, 0, 0.33624200653190767, 0, 0, 0, 0)
    ops.load(94000, 0, 0.32972025358944557, 0, 0, 0, 0)
