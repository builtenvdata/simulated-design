...
def do_nspa_x(max_drift: float = 0.1, dincr: float = 0.001) -> tuple[list[float], list[float]]:
    ...
    ops.load(91000, 0.10411308538440248, 0, 0, 0, 0, 0)
    ops.load(92000, 0.22903819020150465, 0, 0, 0, 0, 0)
    ops.load(93000, 0.3498962294602098, 0, 0, 0, 0, 0)
    ops.load(94000, 0.3169524949538831, 0, 0, 0, 0, 0)
...
def do_nspa_y(max_drift: float = 0.1, dincr: float = 0.001) -> tuple[list[float], list[float]]:
    ...
    ops.pattern('Plain', 2, 2)
    # Add lateral nspa loads to ops domain
    ops.load(91000, 0, 0.11222203259625814, 0, 0, 0, 0)
    ops.load(92000, 0, 0.23180719770810018, 0, 0, 0, 0)
    ops.load(93000, 0, 0.34776723923098896, 0, 0, 0, 0)
    ops.load(94000, 0, 0.30820353046465276, 0, 0, 0, 0)
