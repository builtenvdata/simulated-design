...
def do_nspa_x(max_drift: float = 0.1, dincr: float = 0.001) -> tuple[list[float], list[float]]:
    ...
    ops.load(91000, 0.10326270642444045, 0, 0, 0, 0, 0)
    ops.load(92000, 0.21705601274905195, 0, 0, 0, 0, 0)
    ops.load(93000, 0.34879614064200337, 0, 0, 0, 0, 0)
    ops.load(94000, 0.3308851401845042, 0, 0, 0, 0, 0)
...
def do_nspa_y(max_drift: float = 0.1, dincr: float = 0.001) -> tuple[list[float], list[float]]:
    ...
    ops.pattern('Plain', 2, 2)
    # Add lateral nspa loads to ops domain
    ops.load(91000, 0, 0.1122863123805142, 0, 0, 0, 0)
    ops.load(92000, 0, 0.2234791170753972, 0, 0, 0, 0)
    ops.load(93000, 0, 0.3472750178595754, 0, 0, 0, 0)
    ops.load(94000, 0, 0.31695955268451315, 0, 0, 0, 0)
