...
def do_nspa_x(max_drift: float = 0.1, dincr: float = 0.0001) -> tuple[list[float], list[float]]:
    ...
    ops.load(91000, 0.10866261908937284, 0, 0, 0, 0, 0)
    ops.load(92000, 0.2081595506698952, 0, 0, 0, 0, 0)
    ops.load(93000, 0.33960452431700966, 0, 0, 0, 0, 0)
    ops.load(94000, 0.3435733059237222, 0, 0, 0, 0, 0)
...
def do_nspa_y(max_drift: float = 0.1, dincr: float = 0.0001) -> tuple[list[float], list[float]]:
    ...
    ops.pattern('Plain', 2, 2)
    # Add lateral nspa loads to ops domain
    ops.load(91000, 0, 0.10395535128460348, 0, 0, 0, 0)
    ops.load(92000, 0, 0.20791600355417686, 0, 0, 0, 0)
    ops.load(93000, 0, 0.3448732250645693, 0, 0, 0, 0)
    ops.load(94000, 0, 0.34325542009665033, 0, 0, 0, 0)
