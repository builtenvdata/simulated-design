...
def do_nspa_x(max_drift: float = 0.1, dincr: float = 0.001) -> tuple[list[float], list[float]]:
    ...
    ops.load(91000, 0.1191033980324474, 0, 0, 0, 0, 0)
    ops.load(92000, 0.2312560658468604, 0, 0, 0, 0, 0)
    ops.load(93000, 0.3400075639456604, 0, 0, 0, 0, 0)
    ops.load(94000, 0.30963297217503183, 0, 0, 0, 0, 0)
...
def do_nspa_y(max_drift: float = 0.1, dincr: float = 0.001) -> tuple[list[float], list[float]]:
    ...
    ops.pattern('Plain', 2, 2)
    # Add lateral nspa loads to ops domain
    ops.load(91000, 0, 0.12121793171467252, 0, 0, 0, 0)
    ops.load(92000, 0, 0.23584704978185275, 0, 0, 0, 0)
    ops.load(93000, 0, 0.33836490855345774, 0, 0, 0, 0)
    ops.load(94000, 0, 0.3045701099500169, 0, 0, 0, 0)
