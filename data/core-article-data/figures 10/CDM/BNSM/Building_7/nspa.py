...
def do_nspa_x(max_drift: float = 0.1, dincr: float = 1e-05) -> tuple[list[float], list[float]]:
    ...
    ops.load(91000, 0.12846420290963395, 0, 0, 0, 0, 0)
    ops.load(92000, 0.2320547877662065, 0, 0, 0, 0, 0)
    ops.load(93000, 0.3329991704791839, 0, 0, 0, 0, 0)
    ops.load(94000, 0.30648183884497565, 0, 0, 0, 0, 0)
...
def do_nspa_y(max_drift: float = 0.1, dincr: float = 1e-05) -> tuple[list[float], list[float]]:
    ...
    ops.pattern('Plain', 2, 2)
    # Add lateral nspa loads to ops domain
    ops.load(91000, 0, 0.1275281595085628, 0, 0, 0, 0)
    ops.load(92000, 0, 0.2327702540290188, 0, 0, 0, 0)
    ops.load(93000, 0, 0.3335052356942014, 0, 0, 0, 0)
    ops.load(94000, 0, 0.30619635076821694, 0, 0, 0, 0)
