...
def do_nspa_x(max_drift: float = 0.1, dincr: float = 0.0001) -> tuple[list[float], list[float]]:
    ...
    ops.load(91000, 0.11725328105411358, 0, 0, 0, 0, 0)
    ops.load(92000, 0.2234440884346412, 0, 0, 0, 0, 0)
    ops.load(93000, 0.36170211038224376, 0, 0, 0, 0, 0)
    ops.load(94000, 0.29760052012900146, 0, 0, 0, 0, 0)
...
def do_nspa_y(max_drift: float = 0.1, dincr: float = 0.0001) -> tuple[list[float], list[float]]:
    ...
    ops.pattern('Plain', 2, 2)
    # Add lateral nspa loads to ops domain
    ops.load(91000, 0, 0.1216024055598715, 0, 0, 0, 0)
    ops.load(92000, 0, 0.23819522034466142, 0, 0, 0, 0)
    ops.load(93000, 0, 0.35658731512791636, 0, 0, 0, 0)
    ops.load(94000, 0, 0.2836150589675506, 0, 0, 0, 0)
