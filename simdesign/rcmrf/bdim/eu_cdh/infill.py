...
masonry infill walls for the ``eu_cdh`` design class in the BDIM layer.
"""
...
class Infill(InfillBase):
    """Masonry infill wall implementation for design class ``eu_cdh``.

    This class extends ``InfillBase`` by narrowing the attribute types.

    Attributes
    ----------
    beams : List[~simdesign.rcmrf.bdim.eu_cdh.beam.Beam or None]
