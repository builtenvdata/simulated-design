...
masonry infill walls for the ``eu_cdn`` design class in the BDIM layer.
"""
...
class Infill(InfillBase):
    """Masonry infill wall implementation for design class ``eu_cdn``.

    This class extends ``InfillBase`` by narrowing the attribute types.

    Attributes
    ----------
    beams : List[~simdesign.rcmrf.bdim.eu_cdn.beam.Beam or None]
