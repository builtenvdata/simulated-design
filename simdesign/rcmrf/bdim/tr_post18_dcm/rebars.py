...
detailing of structural members for the``tr_post18_dcm`` design class in the
BDIM layer.
"""
...
class Rebars(RebarsBase):
    """Rebars implementation for design class ``tr_post18_dcm``.

    This class extends ``RebarsBase`` by providing the detailing rules
    appropriate for the ``tr_post18_dcm`` design class.
    ...
    def _get_beam_max_sbh_end(self, **kwargs) -> float | np.ndarray:
        ...
        Notes
        -----
        Based on Section 7.8.4 in TBEC-2018.
        """
    ...
    def _get_col_max_sbh(self, **kwargs) -> float | np.ndarray:
        ...
        Notes
        -----
        Based on Section 7.7.4.1 in TBEC-2018
        """
        # maximum allowed spacing in current iteration
