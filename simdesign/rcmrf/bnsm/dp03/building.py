...
implementation for the ``DP03`` modelling configuration.
"""
...
class Building(BuildingBase):
    """BNSM implementation for the ``DP03`` model.

    This class aggregates DP03-specific structural components (e.g. beams,
    columns, joints, infills) and relies on the behaviour defined in
    ``BuildingBase`` without modification.

    Attributes
    ----------
    foundations : List[~simdesign.rcmrf.bnsm.dp03.foundation.Foundation]
        List of foundation instances.
    floors : List[~simdesign.rcmrf.bnsm.dp03.floor.FloorDiaphragm]
        List of floor instances.
    floor_joints : List[~simdesign.rcmrf.bnsm.dp03.joint.FloorJoint]
        List of floor joints instances.
    stairs_joints : List[~simdesign.rcmrf.bnsm.dp03.joint.StairsJoint]
        List of stairs joints instances.
    beams : List[~simdesign.rcmrf.bnsm.dp03.beam.Beam]
        List of beam instances.
    columns : List[~simdesign.rcmrf.bnsm.dp03.column.Column]
        List of column instances.
    infills : List[~simdesign.rcmrf.bnsm.dp03.infill.Infill]
    ...
    def _find_beam_by_design(self, design: BeamDesign) -> Optional[Beam]:
        ...
        Parameters
        ----------
        design : ~simdesign.rcmrf.bnsm.dp03.beam.BeamDesign
    ...
    def _find_column_by_design(self, design: ColumnDesign) -> Optional[Column]:
        ...
        Parameters
        ----------
        design : ~simdesign.rcmrf.bnsm.dp03.column.ColumnDesign
